...
# ProductUpgrade Frontend Scraper
...
if command -v lighthouse &> /dev/null; then
  for route in $(cat .productupgrade/routes.txt); do
    lighthouse "http://localhost:3000${route}" \
      --output=json \
      --output-path=".productupgrade/lighthouse/${route//\//_}.json" \
...
Write output to `.productupgrade/AUDIT-FRONTEND.md`
