...
Every agent in `/ultra-upgrade` and `/auto-swarm deep` mode receives a composed prompt built from these 7 layers. Layers are applied selectively based on the agent's role and the current iteration context.
